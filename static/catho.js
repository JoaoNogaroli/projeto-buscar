            ...
            console.log("TAMANHO LISTA 1:" + lista.length)
                ...
                if (a =1){
                    var interv = setInterval(function repetir() {
                        ...
                        if(lista.length==0){
                            database.ref('Users').child(user_uid).child('Pesquisa_catho').child('Lista_resultados_catho').on('value', function(snapshot){
                                snapshot.forEach(function(data) {  
                                    database.ref('Users').child(user_uid).child('Pesquisa_catho').child('Lista_resultados_catho').child(data.key).on('value', function(snapshot){
                            ...
                            database.ref('Users').child(user_uid).child('Pesquisa_catho').child('Lista_resultados_catho').on('value', function(snapshot){
                                snapshot.forEach(function(data) {  
                                    database.ref('Users').child(user_uid).child('Pesquisa_catho').child('Lista_resultados_catho').child(data.key).on('value', function(snapshot){
                                        ...
                                        var snap_valor = snapshot.val().ValorDaVaga;
                                        ...
                                        var li_valor = document.createElement('h2');
                                        li_valor.textContent= snap_valor;
                                        li_valor.className = 'li_valor'
                                        div.appendChild(li_valor)
